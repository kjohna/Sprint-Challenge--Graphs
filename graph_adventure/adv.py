...
def bfs_nearest(roomId, traversalPath, traversalGraph):
    ...
    q = queue.Queue()
    q.put(roomId)
    # need a place to keep track of each room's "parent"
    parents = {}
    # this will be helpful later:
    opposite = {'n': 's', 'e': 'w', 's': 'n', 'w': 'e'}
    # a place to store our route to '?' once we find one
    routeToQ = []
    while not q.empty():
        room = q.get()
        for ex in traversalGraph[room]:
            exitsTo = traversalGraph[room][ex]
            if exitsTo == '?':
                # build directions to the nearest '?'
                # look up where we came from until we get back to current room
                while not room == roomId:
                    for ex2 in traversalGraph[room]:
                        if traversalGraph[room][ex2] == parents[room]:
                            routeToQ.append(opposite[ex2])
                            room = parents[room]
                            # no need to check more exits
                            break
                # once route is built, empty q and break
                while not q.empty():
                    q.get()
                break
            else:
                # if exit goes to a room we haven't checked,
                # put on q, log that room's parent
                if not exitsTo in parents:
                    q.put(exitsTo)
                    parents[exitsTo] = room
    # if we have a route, travel that route, otherwise done!
    if len(routeToQ) > 0:
        for direction in routeToQ:
